...
```{r}
results_table(enticement1)
```

```{r}
results_table(enticement2)
```
...
```{r}
results_table(valence1)
```

```{r}
results_table(valence2)
```
...
model_draft <- glmmTMB::glmmTMB(Arousal ~ Gender / Condition + (Condition|Participant) + (1|Item),
                          data=df2,
