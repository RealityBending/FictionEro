...
results_table(arousal1)

pred_a1 <- estimate_means(arousal1, by = c("Sex", "Relevance", "Condition", "ConditionBelief"), backend = "emmeans")  |>
  as.data.frame()

pred_a1|>
  mutate(Sex = fct_relevel(Sex, "Male", "Female")) |>
  ggplot(aes(x = Arousal, y= Proportion ))

```

```{r}
arousal2 <- readRDS("models/ModelArousal_2.rds")
results_table(arousal2)

pred_a2 <- estimate_means(arousal2, by = c("Gender","Condition", "ConditionBelief"), backend = "emmeans") |>
  as.data.frame()
```
