...
df <- read.csv("../data/data.csv") |> 
  right_join(
    select(dfsub, Participant, Mobile, starts_with(c("Feedback_","BAIT")), COPS_Frequency, SexualActivity, BAIT_Worry, BAIT_Dangerous, 
           -Feedback_Text),
    by = "Participant"
    ) |> 
...
raw_scores <- data.frame(
  Participant = df[!is.na(df$BAIT_ImagesRealistic), ]$Participant,
  BAIT_Videos = (bait$BAIT_VideosRealistic + (1 - bait$BAIT_VideosIssues)) / 2,
  BAIT_Visual = (bait$BAIT_ImagesRealistic + (1 - bait$BAIT_ImagesIssues) + bait$BAIT_ImitatingReality + bait$BAIT_EnvironmentReal) / 4,
  BAIT_Text = (bait$BAIT_TextRealistic + (1 - bait$BAIT_TextIssues)) / 2
  )
...
for(m in c("BAIT_Visual", "BAIT_Text", "BAIT_ExpertAI", "PornFrequency", "SexualActivity")) {
...
```{r}
  m <- glmmTMB::glmmTMB(Arousal ~ Sex / Condition + (1|Participant) + (1|Item),
