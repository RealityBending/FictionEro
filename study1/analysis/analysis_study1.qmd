...
```{r}
GAAIS_Pos_mean <- rowMeans(dfsub1[, grepl("^GAAIS_Positive_[0-9]+$", names(dfsub1))], na.rm = TRUE)
GAAIS_Neg_mean <- rowMeans(dfsub1[, grepl("^GAAIS_Negative_[0-9]+$", names(dfsub1))], na.rm = TRUE)

dfsub1$BAIT_Attitudes <- (GAAIS_Pos_mean + GAAIS_Neg_mean) / 2
...
```{r}
Text_mean   <- rowMeans(BAIT_Text_items, na.rm = TRUE)
Visual_mean <- rowMeans(BAIT_Visual_items, na.rm = TRUE)
Videos_mean <- rowMeans(BAIT_Videos_items, na.rm = TRUE)

dfsub1$BAIT_Expectations <- (Text_mean + Visual_mean + Videos_mean)
...
df1 |>
  group_by(Gender, Relevance, Condition)|>
  summarise(n=n(),
...
ttest_less_arousal <- BayesFactor::ttestBF(
  formula = diff_arousal ~ Feedback_AILessArousing,
  data = dfsub1_arousal |> tidyr::drop_na(diff_arousal, Feedback_AILessArousing)
  
ttest_less_arousal
# ttest_less_arousal@data |> nrow()  

post <- BayesFactor::posterior(ttest_less_arousal, iterations = 10000)
bayestestR::describe_posterior(as.data.frame(post)$delta)
...
ttest_less_enticement <- BayesFactor::ttestBF(
  formula = diff_enticement ~ Feedback_AILessArousing,
  data = dfsub1_enticement |> tidyr::drop_na(diff_enticement, Feedback_AILessArousing)
)
ttest_less_enticement
# ttest_less_enticement@data |> nrow()   

post <- BayesFactor::posterior(ttest_less_enticement, iterations = 10000)
bayestestR::describe_posterior(as.data.frame(post)$delta)
```
...
ttest_less_valence <- BayesFactor::ttestBF(
  formula = diff_valence ~ Feedback_AILessArousing,
  data = dfsub1_valence |> tidyr::drop_na(diff_valence, Feedback_AILessArousing)
)
ttest_less_valence
# ttest_less_valence@data |> nrow()   

post <- BayesFactor::posterior(ttest_less_valence, iterations = 10000)
bayestestR::describe_posterior(as.data.frame(post)$delta)
```
...
pred_a3 <- pred_a3 |>
  mutate(
    ImageContent = case_when(
      Relevance == "NonErotic" ~ "NonErotic",
      Gender == "Male"   & Relevance == "Relevant"   ~ "Female Images",
      Gender == "Male"   & Relevance == "Irrelevant" ~ "Male Images",
      Gender == "Female" & Relevance == "Relevant"   ~ "Male Images",
      Gender == "Female" & Relevance == "Irrelevant" ~ "Female Images"
    ),
    ImageContent = fct_relevel(ImageContent, "Female Images", "Male Images", "NonErotic")
  )
...
fig2 <- pred_a3 |>
  ...
  ggplot(aes(x = ImageContent, y = Probability)) +
  geom_hline(yintercept = 0, color = "darkgrey") +
  geom_line(aes(group = Condition, color = Condition),
            position = position_dodge(width = 0.4), alpha = 0.8, linewidth = 0.2) +
  geom_pointrange(aes(ymin = CI_low, ymax = CI_high, color = Condition),position = position_dodge(width = 0.4), size = 0.6) +
  geom_rect(data = data.frame(Probability = 0.1, Gender = as.factor("Male"), ConditionBelief = "Congruent", ImageContent = "Female Images"),
            aes(xmin = 0.8, xmax = 1.2, ymin = 0.45, ymax = 0.75), color = "#8BC34A", alpha = 0, linewidth = 1.2) +
  geom_rect(data = data.frame(Probability = 0.1, Gender = as.factor("Female"), ConditionBelief = "Congruent", ImageContent = "Male Images"),
            aes(xmin = 1.8, xmax = 2.2, ymin = 0.20, ymax = 0.50), color = "#8BC34A", alpha = 0, linewidth = 1.2) +
  geom_rect(data = data.frame(Probability = 0.1, Gender = as.factor("Male"), ConditionBelief = "Incongruent", ImageContent = "Female Images"),
            aes(xmin = 0.8, xmax = 1.2, ymin = 0.40, ymax = 0.70), color = "#8BC34A", alpha = 0, linewidth = 1.2) +
  geom_rect(data = data.frame(Probability = 0.1, Gender = as.factor("Female"), ConditionBelief = "Incongruent", ImageContent = "Male Images"),
            aes(xmin = 1.8, xmax = 2.2, ymin = 0.20, ymax = 0.50), color = "#8BC34A", alpha = 0, linewidth = 1.2) +
  geom_rect(data = data.frame(Probability = 0.1, Gender = as.factor("Female"), ConditionBelief = "Congruent", ImageContent = "Female Images"),
            aes(xmin = 0.8, xmax = 1.2, ymin = 0.16, ymax = 0.46), color = "#8BC34A", alpha = 0, linewidth = 1.2) +
  geom_rect(data = data.frame(Probability = 0.1, Gender = as.factor("Female"), ConditionBelief = "Incongruent", ImageContent = "Female Images"),
            aes(xmin = 0.8, xmax = 1.2, ymin = 0.16, ymax = 0.46), color = "#8BC34A", alpha = 0, linewidth = 1.2) +
  facet_grid(Gender ~ ConditionBelief, switch = "y") +
  geom_rect(data = data.frame(Probability = 0.1, Gender = as.factor("Male"), ConditionBelief = "Congruent", ImageContent = "NonErotic"),
...
correlations <- df1 |>
  select(Participant, BAIT_Attitudes, BAIT_Expectations, starts_with("Porn"), "AI_Knowledge", "SexualActivity_num", "COPS_Frequency_2") |>
