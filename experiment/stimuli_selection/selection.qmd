...
men <- df |>
  filter(Type == "Erotic") |>
  group_by(Category) |>
  slice_max(Men_Arousal, n=6, with_ties = FALSE) |>
  pull(ID) |> 
  c(
    df |>
      filter(Type == "Erotic") |>
...
women <- df |>
  ...
  group_by(Category) |>
  slice_max(Women_Arousal, n=6, with_ties = FALSE) |>
  pull(ID) |> 
  c(
    df |>
      filter(Type == "Erotic", !ID %in% men) |>
...
neutral <- df |>
  ...
  slice_min(Arousal, n=12, with_ties = FALSE) |>
  pull(ID)
...
pos_arousing <- df |>
  ...
  filter(Valence > 5) |>
  slice_max(Arousal, n=12, with_ties = FALSE) |>
  pull(ID)
