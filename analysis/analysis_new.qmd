...
colors <- c(
  "Intercept" = "#9C27B0",                   
  "RelevanceIrrelevant" = "#1E88E5",   
  "RelevanceNonErotic"  = "#43A047", 
  "RelevanceIrrelevant:ConditionAIGenerated" = "#00897B",
  "RelevanceRelevant:ConditionAIGenerated"  = "#E53935",
  "RelevanceNonErotic:ConditionAIGenerated" = "#FF9800")
...
colors2 <- c("Intercept" = "#9C27B0", "ConditionAIGenerated" = "#FF9800")
...
df_diff_long <- dfsub1 |>
  right_join(
    df1 |>
      ...
      pivot_wider(
        ...
        names_sep = "_") |>
      mutate(
        diff_arousal = mean_arousal_AIGenerated - mean_arousal_Photograph,
  ...
  pivot_longer(
    ...
    values_to = "Diff") |>
  mutate(
    Outcome = factor(Outcome, levels = c("diff_arousal", "diff_enticement", "diff_valence"),
                     labels = c("Arousal", "Enticement", "Valence")),
    Feedback_AILessArousing = as.factor(Feedback_AILessArousing))

# Plot
diff_plot_long <- df_diff_long |>
  ggplot(aes(x = Feedback_AILessArousing, y = Diff, fill = Feedback_AILessArousing)) +
  geom_boxplot(width = 0.4, alpha = 0.7) +
  geom_jitter(width = 0.2, alpha = 0.1, size = 1) +
  stat_summary(fun = mean, geom = "point", shape = 23, size = 2, fill = "black") +
...
#   mutate(
#     Gender = fct_relevel(Gender, "Male", "Female"),
#     Outcome = factor(Outcome, levels = c("Arousal", "Enticement", "Valence"))
#   )
# 
# p_interactions <- ggplot(
#   pred_interactions,
#   aes(x = Relevance, y = Estimate, color = Condition, group = Condition)
# ) +
#   geom_hline(yintercept = 0, color = "grey80") +
#   geom_line(position = position_dodge(width = 0.2), linewidth = 0.3) +
#   geom_pointrange(
...
#   scale_color_manual(
#     values = c("AIGenerated" = "#2196F3", "Photograph" = "#F44336")
#   ) +
#   labs(
#     x = "Image Relevance",
#     y = "Predicted Rating",
