...
*Note*. Models for study 1 only include the conditions which were relevant to the participants sexual orientation (e.g., seeing images of men as a heterosexual woman). 
...
tbl_results <-   function(model1, title_table = "Fixed Effects: Study 1"){      
  ...
  # ---- Fixed effects table ----
  tbl <- parameters::parameters(model1, effects = "fixed") |>
    as.data.frame() |>
    select(-Effects, -df_error, -CI) |>
    gt() |>
...
```{r}
plot_grouplevel_effects <- function(model, outcome_name = NULL) {
  
  # Colors
  colors <- c("Participant" = "#9C27B0", "Item" = "#FF9800")
  
  # Plot
  p <- ggplot(model, aes(x = Median, y = Level)) +
    geom_pointrange(aes(xmin = CI_low, xmax = CI_high, color = Group), size = 0.3) +
    ggside::geom_xsidedensity(aes(group = Group, fill = Group), color = NA, alpha = 0.3) +
    ...
    labs(
      x = ifelse(is.null(outcome_name), "Random intercept (participant/item)", paste0(outcome_name, " random intercept")),
      y = "",
      color = "Group",
      fill = "Group"
    )
...
plot_grouplevel_effects(a1, outcome_name = "Arousal")
