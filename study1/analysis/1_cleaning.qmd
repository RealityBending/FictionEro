...
`r length(outliers$expe_duration)` participants completed the experiment in less than 15 minutes.
...
`r length(outliers$bait_duration)` participants completed the BAIT questionnaire in less than 30 seconds.
...
We removed `r sum(df$Sex=="Other" | df$SexualOrientation=="Other")` participants that were incompatible with further analysis (i.e., sex and sexual orientation as "other").
...
We kept only heterosexual participants (`r insight::format_percent(sum(df$SexualOrientation == "Heterosexual") / nrow(df))`). Therefore we removed  participants that identified: as Bisexual (N= (`r sum(df$SexualOrientation == "Bisexual")`), or Homosexual (N=`r `sum(df$SexualOrientation == "Homosexual")` ) or Other (N=`r sum(df$SexualOrientation == "Other")`). 
...
n <- parameters::n_factors(bait)
plot(n)

efa <- parameters::factor_analysis(bait, n = 3, rotation = "oblimin", scores = "tenBerge", use = "pairwise", factor_method = "ml")
