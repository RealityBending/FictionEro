...
```{r}
arousal1_GAU <- readRDS("models/Gaussian/ModelArousal_1.rds")
arousal2_GAU <- readRDS("models/Gaussian/ModelArousal_2.rds")

arousal1_ZOIB <- readRDS("models/ZOIB/ModelArousal_1_ZOIB.rds")
arousal2_ZOIB <- readRDS("models/ZOIB/ModelArousal_2_ZOIB.rds")

arousal1_ZOIB_sub <- readRDS("models/ZOIB/subset/ModelArousal_1_ZOIB_subset.rds")
arousal2_ZOIB_sub <- readRDS("models/ZOIB/subset/ModelArousal_2_ZOIB_subset.rds")
...
```{r}
params_table(arousal1_GAU)
summary(arousal1, robust = TRUE)
```

```{r}
params_table(arousal2_GAU) 
...
params_table(arousal1_ZOIB_sub)
summary(arousal1_ZOIB_sub, robust = TRUE)
```
...
params_table(arousal2_ZOIB_sub) 
summary(arousal2_ZOIB_sub, robust = TRUE)
```
...
```{r}
performance::performance(arousal1_GAU)
```

```{r}
performance::performance(arousal2_GAU)
```
...
```{r}
performance::performance(arousal2_ZOIB)
```
:::
...
```{r}
a1 <- modelbased::estimate_grouplevel(arousal1_GAU)
a2 <- modelbased::estimate_grouplevel(arousal2_GAU)
```
