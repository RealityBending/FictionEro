...
m_e_b <- glmmTMB::glmmTMB(Enticing ~ Gender / Condition + (Condition| Participant) + (1|Stimulus),
                      data=filter(df, ConditionBelief == "True"), 
                      family=glmmTMB::ordbeta(),
                      control = glmmTMB::glmmTMBControl(parallel = 8))
...
Looking at avergae slopes, both genders rate AI-generated images lower in valence, but this effect is only significant for males. 
...
```{r Valence Believed1}
m_v_b <- glmmTMB::glmmTMB(Valence ~ Gender /Condition + (Condition| Participant) + (1|Stimulus),
                      data=filter(df, ConditionBelief == "True"), 
                      family=glmmTMB::ordbeta(),
                      control = glmmTMB::glmmTMBControl(parallel = 8))
...
m_em <- glmmTMB::glmmTMB(Emotionality ~ Gender /StimuliType/ Condition + (Condition| Participant) + (1|Stimulus),
                      data=df, family=glmmTMB::ordbeta(),
                      control = glmmTMB::glmmTMBControl(parallel = 8))

results_table(m_em)

marginaleffects::avg_slopes(m_em, variables="Condition", by="Gender", re.form=NA) |>
  results_table()
```
...
pred1 <- rbind(
  marginaleffects::avg_predictions(m_arousal, by=c("Gender", "StimuliType", "Condition"), newdata = "mean", re.form=NA) |>
  as.data.frame() |> 
  mutate(Outcome="Arousal"),
  marginaleffects::avg_predictions(m_enticement, by=c("Gender","StimuliType",  "Condition"), newdata = "mean", re.form=NA) |>
  as.data.frame() |> 
  mutate(Outcome="Enticement"),
  marginaleffects::avg_predictions(m_valence, by=c("Gender", "StimuliType", "Condition"), newdata = "mean", re.form=NA) |>
  as.data.frame() |> 
  mutate(Outcome="Valence")
)

stars1a <- data.frame(label=c("**","***", "**", "***", "***"),
                    Gender=fct_relevel(c("Male", "Female", "Male", "Female", "Female"), "Male"),
                    Outcome=c("Arousal","Arousal", "Enticement", "Enticement", "Valence"),
                    Condition=c("Photograph", "Photograph", "Photograph", "Photograph", "Photograph"), 
                    x=c(1.5, 1.5, 1.5, 1.5, 1.5), 
                    y = c(0.52, 0.40, 0.57, 0.51, 0.6))

stars1b <- data.frame(label=c("*","**", "*","*"),
                    Gender=fct_relevel(c("Male", "Male", "Male","Female"), "Male"),
                    Outcome=c("Arousal", "Enticement", "Enticement","Valence"),
                    x=c(1, 1, 2, 2),
                    y=c(0.58, 0.63, 0.54, 0.6))


p1 <- pred1 |>
  mutate(Gender = fct_relevel(Gender, "Male", "Female")) |> 
  ggplot(aes(x=StimuliType, y=estimate)) +
  geom_hline(yintercept=0, color="darkgrey") +
  geom_line(aes(group=Condition, color=Condition), position = position_dodge(width=0.2), alpha=0.8, linewidth=0.2, key_glyph = "path") +
  geom_pointrange(aes(ymin = conf.low, ymax=conf.high, color=Condition), position = position_dodge(width=0.2), size=0.3) +
  geom_text(data=stars1a, aes(x=x, y=y, label=label, color=Condition), hjust=0.5, key_glyph = "blank", size=3) +
  geom_text(data=stars1b, aes(x=x, y=y, label=label), hjust=0.5, color="#424242", size=3) +
...
stars2 <- data.frame(label=c("**", "***", "*"),
                    Gender=fct_relevel(c("Male", "Male", "Male")),
                    Type=c("All trials", "Believed trials" ,"'AI was more attractive'"),
                    y=c(0.52, 0.56, 0.66))
