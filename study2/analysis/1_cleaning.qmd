...
dftask <- read.csv("../data/rawdata_task.csv") |>
    full_join(
        df[c("Participant", "Gender", "SexualOrientation")], # add Gender and sexual orientation to this dataset
...
df |>
    ...
    filter(Answer == "True") |>
    mutate(
        Question = str_remove(Question, "Feedback_"),
...
df |>
    ...
    drop_na() |>
    mutate(
        Authenticity = str_remove(Authenticity, "Feedback_"),
...
There were `r  length(outliers$Mobile)` participants that used a mobile device.
...
df |>
    mutate(
        Participant = fct_reorder(Participant, Bait_Duration),
...
dat |>
    ...
    mutate(
        name = fct_relevel(name, "cor_ArVal", "cor_ArEnt"),
...
Only `r sum(df$SexualOrientation == "Other")` participants self-reported as not being heterosexual, homosexual, nor bisexual, hence they were removed from further analysis. 

```{r}
df <- filter(df, !Gender == "Other")
df <- filter(df, !SexualOrientation == "Other")
