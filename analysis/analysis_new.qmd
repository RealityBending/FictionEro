...
```{r}
m_log <- glmmTMB::glmmTMB(ConditionBelief ~ Gender * Type + (1 | Participant), data = df2, family = binomial)
summary(m_log)
```
...
```{r arousal 2 est cont}
estimate_contrasts(m_type, contrast = "Condition", by=c("Gender", "ConditionBelief"), backend = "emmeans")
```
...
pd <- data.frame(label=c("pd = 100%", "pd = 97.18%", "pd = 99.71%"),
...
pred_a2 |>
  ...
  geom_rect(data=data.frame(Probability=0.1, Gender=as.factor("Female"), ConditionBelief="Congruent"),
            aes(xmin = 0.8, xmax = 1.2,ymin=0.3, ymax=0.50), color="#8BC34A", alpha=0, linewidth = 1.2) +
