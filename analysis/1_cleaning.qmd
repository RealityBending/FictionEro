...
df <- read.csv("../data/rawdata_participants.csv") |> 
  ...
  mutate(Sample = case_when(
    Language=="English" & str_detect(Experimenter, "reddit") ~ "Reddit",
    Language=="English" & Experimenter %in% c("2discord1", "Experimenter2", "Experimenter1") ~ "Student",
    Language=="English" & Experimenter %in% c("Unknown", "Readme (GitHub)", "website") ~ "Other",
...
table_recruitment <- function(df) {
  summarize(df, N = n(), .by=c("Language", "Sample")) |> 
    arrange(desc(N)) |> 
    gt::gt() |> 
    gt::opt_stylize() |> 
