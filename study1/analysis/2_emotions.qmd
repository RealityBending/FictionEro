...
<!-- Note: The model did not converge when adding Relevance and Condition as random slopes for Item. -->
...
<!-- Note. Not enough contrasts to add relevance and condition as random slopes for items -->
```{r}
...
m_enticement <- glmmTMB::glmmTMB(Enticement ~ Sex / Relevance / Condition + 
                                   (Relevance | Participant) + ((Relevance/Condition)|Item),
                      data=df, family=glmmTMB::ordbeta(),
                      control = glmmTMB::glmmTMBControl(parallel = 8))
...
pred1 <- 
  rbind(
  modelbased::estimate_means(m_arousal, by=c("Sex", "Relevance", "Condition")) |>
  as.data.frame() |> 
  mutate(Outcome="Arousal"),
  modelbased::estimate_means(m_enticement, by=c("Sex", "Relevance", "Condition")) |>
  as.data.frame() |> 
  mutate(Outcome="Enticement"),
  modelbased::estimate_means(m_valence, by=c("Sex", "Relevance", "Condition")) |>
  as.data.frame() |> 
