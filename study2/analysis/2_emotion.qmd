...
df$SexualOrientation <- droplevels(df$SexualOrientation)
...
This model examines the effect of **Gender Sexual Orientation and Condition on Arousal**, accounting for random variability due to participants and items (i.e., random intercepts) and condition as random slope for participants.

<!-- The intercept is Male, photograph, individuals  -->

Men rate images labelled as AI-generated as less arousing than images labelled as real.  Women also rate AI-generated images lower in arousal but not significantly. 
...
The analysis revealed a significant interaction effect for **positive, and negative, attitudes towards AI** among females and males respectively. Specifically, females with higher positive attitudes towards AI rated AI-generated images as more arousing, relative to when they viewed images labelled as photographs. Additionally, males with higher **negative attitudes towards AI** rated AI-generated labelled images as less arousing.

Females self-report on their expertise on AI was also a moderator of arousal ratings. In particular, females with higher ratings of AI-knowledge rated images labelled as AI-generated as more arousing. 
...
We also examined whether **feedback** moderates the relationship between gender and condition on arousal.

Findings suggest that feedback moderates this relationship only for men. Specifically, males who reported finding AI-generated images as less and more arousing had lower and higher, respectively, arousal ratings for those images.
...
### Believed
#### With Stimuli Type
```{r Arousal Believed with stimuli type}
...
#### Without Stimuli Type

For trials believed to be true (i.e., labelled in phase 2 matches label presented in phase 1), both males and females rated AI-generated images lower in Arousal then images labelled as real photos.

```{r Arousal Believed without stimuli type}
...
Men rate images presented as AI-generated significantly lower in enticement than images presented as real photos.
...
```{r Enticement Model with stimuli type}
...
The results showed that enticing ratings for images labeled AI-generated were moderated by the feedback reporting attitudes towards AI. Specifically, women who reported having more positive attitudes towards AI, rated these images higher in arousal compared to images labeled as photographs. While, men reporting having more negative attitudes towards AI reported these images as less enticing. 

Sexual activity was a significant moderator for females ratings of enticement, specifically those that reported having has sexual acitvity in the three days prior to the survey rated images as more enticing. 
...
Looking at **feedback as a moderator**, the results showed that enticement ratings for images labelled as AI-Generated were moderated by the feedback that AI is less attractive for males, where they rated these images as less enticing. Additionally, women who reported believing all labels were incorrect rated AI-generated images as less enticing. 
...
```{r Enticement Believed1 with stimuli type}
...
For trials where the original labels where believed to be true both genders rated AI-generated images as less enticing. 

```{r Enticement Believed1 wihtout stimuli type}
...
Both genders rate AI-generated images as less valent than images labelled as real photos. 
...
Here we look at whether the relationship between gender and condition and valence is moderated by **AI-attitudes, porn frequency, and sexual activity**. 

The results showed that enticing ratings for images labeled AI-generated were moderated by the feedback reporting attitudes towards AI only for males. More specifically, males reporting more negative attitudes towards AI, rated these images as less valent. Additionally, males reporting higher AI experitise, rated AI images as more valent.
...
Looking at **feedback** as a moderator, the results show that valence ratings for images labelled AI-Generated were moderated by the feedback that AI images were more attractive for males. 

Additionally, both feedback that there were no difference between AI and real photos and that the difference between AI and photos was obvious where both moderators for females ratings of valence - with females reporting these feedbacks rating AI images as less valent. 

Males feedback that labels were incorrect was also a moderator for valent ratings of AI-generated images, with these images being rated as more valent for those reporting this feedback.
...
#### With Stimuli Type
```{r Valence Believed1 with stimuli type}
...
Both genders rate images labelled as AI-generated as less valent for trials where they believed the original labels to be true.

```{r Valence Believed1 without stimuli type}
...
Both genders rate AI-generated images as less emotional than images labelled as photographs. 
...
Females, compared to males, report significantly lower emotionality ratings for fotographs. Among males, couple stimuli are rated lower than individual stimuli. Among females, couple stimuli are rated higher than individual stimuli. For males, AI-generated individual, and couple, stimuli are rated lower than human-generated ones. 

```{r Emotionality Model with stimuli type}
m_emotionality_st <- glmmTMB::glmmTMB(Emotionality ~ Gender /StimuliType/ Condition + (Condition| Participant) + (1|Stimulus),
                      data=df, family=glmmTMB::ordbeta(),
                      control = glmmTMB::glmmTMBControl(parallel = 8))

results_table(m_emotionality_st)
...
Looking at **AI-attitudes, porn frequency and sexual activity** as moderators, we can see that for males and females reporting higher expertise in AI led to an increase of emotionality ratings for images labelled as AI-Generated. 

Additionally, males and females who reported having more positive attitiudes towards AI, rated AI-Generated images higher in emotionality compared to fotographs. Additionally, for males, those with more negative AI attitudes rate AI-generated images lower in emotionality.
...
For females, who reproted believing the difference between AI and photos was none, rated AI images higher in emotionality. 
...
#### With Stimuli Type
```{r EMotionality Believed1 with stimuli type}
...
Both genders rated AI-generated images lower in emotionality than images presented as real photos. 

```{r EMotionality Believed1 without stimuli type}
...
-   Gender is a major predictor of arousal, enticement and valence, with females generally reporting lower ratings than males overall but not significantly.
-   Condition is a major predictor of arousal, enticement and valence for males, with images presented as AI-generated being rated lower than images presented as real photos. For females, condition was only a significant predictor for valence scores. 
-   However when taking into account only the trials in which the original labels were believed to be true, both genders rated AI-generated images as less arousing, enticing and valent. 
-  Women prefer couple images while men prefer individual images. 
  
**Moderators**
AI Attitudes:
  - Females with positive attitudes rated AI images as more arousing and enticing.
  - Males with negative attitudes rated AI images as less arousing, enticing, and valent.

AI Expertise:
  - Females with higher expertise rated AI images as more arousing.
  - Males with higher expertise rated AI images as more valent.

Feedback Effects:
  - Males: Feedback that AI images were more arousing, attractive, or mislabelled increased ratings.
  - Females: Feedback suggesting no/obvious difference reduced valence ratings.
...
p1 <- pred1 |>
  ...
  geom_text(data=stars1a, aes(y=y, label=label), hjust=0.5, color="#424242", size=6) +
  geom_rect(data=data.frame(Outcome = "Arousal", Proportion=0.1, Gender=as.factor("Male")),
            aes(xmin=0.8, xmax=1.2, ymin=0.35, ymax=0.53), color="#8BC34A", alpha=0, ) +
  geom_rect(data=data.frame(StimuliType = "Individual",Proportion=0.1, Gender=as.factor("Female"), Outcome="Arousal"),
            aes(xmin=0.8, xmax=1.2, ymin=0.33, ymax=0.48), color="#8BC34A", alpha=0) +
  facet_grid(~Gender, switch="y", scales ="free") +
  scale_y_continuous(limits=c(0.2, 0.7), labels=scales::percent) +
