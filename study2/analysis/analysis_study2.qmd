...
df2 <- read.csv("https://raw.githubusercontent.com/RealityBending/FictionEro/refs/heads/main/study2/data/data.csv") |> 
  right_join(
    select(dfsub2, Participant, Mobile, starts_with(c("Feedback_","BAIT")), COPS_Frequency, SexualActivity,- Feedback_Text, SexualActivity),
    by = "Participant") |> 
...
dfsub2 <- dfsub2 |>
  mutate(COPS_Frequency = case_when(
    ...
    SexualActivity = case_when(
      SexualActivity == "less than 24h ago" ~ 1 ,
      SexualActivity == "within the last 3 days" ~ 2,
      SexualActivity == "within the last week" ~ 3,
      SexualActivity == "within the last month" ~ 4,
      SexualActivity == "within the last year" ~ 5,
      SexualActivity == "more than a year ago" ~ 5)
    )
...
```{r}
inc <- table(df2$ConditionBelief)["True"] /sum(table(df2$ConditionBelief))
```
...
ttest2 <- BayesFactor::ttestBF(
  formula = diff_arousal ~ Feedback_AILessArousing,
  data = dfsub2_arousal |> tidyr::drop_na(diff_arousal, Feedback_AILessArousing)
)

ttest2
# ttest2@data |> nrow()   

post <- BayesFactor::posterior(ttest2, iterations = 10000)
...
ttest2 <- BayesFactor::ttestBF(
  ...
  data = dfsub2_enticement |> tidyr::drop_na(diff_enticement, Feedback_AILessArousing)
)
...
ttest2 <- BayesFactor::ttestBF(
  ...
  data = dfsub2_valence |> tidyr::drop_na(diff_valence, Feedback_AILessArousing)
)
...
```{r}
fig1 <- (arousal_dist2 & theme(axis.text.x = element_blank(),
                               axis.ticks.x = element_blank())) / 
        (enticement_dist2 & theme(axis.text.x = element_blank(),
                                   axis.ticks.x = element_blank())) /valence_dist2 +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom")

ggsave("../../paper/images/Figure4.png", fig1,  width=10, height=12, dpi=300)
```
...
fig6 <- pred_a2 |>
  ...
  geom_rect(data=data.frame(Probability=0.1, Gender=as.factor("Male"), ConditionBelief="Congruent"),
            aes(xmin = 1.8, xmax = 2.2,ymin=0.35, ymax=0.55), color="#8BC34A", alpha=0, linewidth = 1.2) +
  ...
  theme_minimal() +
    theme(axis.title.y = element_blank(),
        ...
        axis.text.x = element_text(hjust=1, vjust=1.1, size=10, color="black"),
        legend.text = element_text(size = 10),
        legend.position = "bottom") +
  labs(color="Images presented as:")

ggsave("../../paper/images/Figure6.png", fig6, width = 10, height = 12, dpi = 300)
...
corr_a <- corr_mod(m, correlations2)
corr_a
```
...
m_type_a <- readRDS("models/ZOIB/ModelArousal_2_ZOIB_type.rds")

```

```{r arousal 2 type model analysis}
params_table(m_type_a)
summary(m_type_a, robust = TRUE)
...
```{r arousal 1 est of type }
#| message: false
#| warning: false

estimate_contrasts(m_type_a, contrast = "Gender", by=c("Type"), backend = "emmeans") 

estimate_contrasts(m_type_a, contrast = "Type", by="Gender", backend = "emmeans")
...
estimate_contrasts(m_type_a, contrast = "Condition", by=c("Gender", "Type"), backend = "emmeans") 
```
...
estimate_contrasts(m_type_a, contrast = "Condition", by=c("Gender", "Type", "ConditionBelief"), backend = "emmeans")
```
:::
...
m_type_e <- readRDS("models/ZOIB/ModelEnticement_2_ZOIB_type.rds")
```

```{r enticement 2 type model analysis}
params_table(m_type_e)
summary(m_type_e, robust = TRUE)
# performance::performance(m)
```
...
```{r enticement 1 est of type }
#| message: false
#| warning: false

estimate_contrasts(m_type_e, contrast = "Gender", by=c("Type"), backend = "emmeans") 
```
...
estimate_contrasts(m_type_e, contrast = "Condition", by=c("Gender", "Type"), backend = "emmeans") 
...
estimate_contrasts(m_type_e, contrast = "Condition", by=c("Gender", "Type", "ConditionBelief"), backend = "emmeans")
```
:::
...
m_type_v <- readRDS("models/ZOIB/ModelValence_2_ZOIB_type.rds")
```

```{r valence 2 type model analysis}
params_table(m_type_v)
summary(m_type_v, robust = TRUE)
# performance::performance(m)
```
...
```{r valence 1 est of type }
#| message: false
#| warning: false

estimate_contrasts(m_type_v, contrast = "Gender", by=c("Type"), backend = "emmeans") 
...
estimate_contrasts(m_type_v, contrast = "Condition", by=c("Gender", "Type"), backend = "emmeans") 
```

#### Effect of Condition depending on congruency

```{r valence  2 type est cont}
estimate_contrasts(m_type_v, contrast = "Condition", by=c("Gender", "Type", "ConditionBelief"), backend = "emmeans")
```
:::
...
pred <- rbind(
  modelbased::estimate_means(m_type_a, by=c("Gender", "Type"), backend = "emmeans") |>
  as.data.frame() |> 
  mutate(Outcome="Arousal"),
  modelbased::estimate_means(m_type_e, by=c("Gender", "Type"), backend = "emmeans") |>
  as.data.frame() |> 
  mutate(Outcome="Enticement"),
  modelbased::estimate_means(m_type_v, by=c("Gender","Type"), backend = "emmeans") |>
  as.data.frame() |> 
...
type_fig <- pred |>
  ...
  geom_hline(yintercept=0, color="darkgrey") +
  geom_line(aes(group=Gender, color=Gender), position = position_dodge(width=0.2), alpha=0.8, linewidth=0.2, key_glyph = "path") +
  geom_pointrange(aes(ymin = CI_low, ymax=CI_high, color=Gender), position = position_dodge(width=0.2), size=0.6) +
  # geom_text(data=stars1a, aes(y=y, label=label), hjust=0.5, color="#424242", size=6) +
  geom_rect(data=data.frame(Outcome = "Arousal", Probability=0.1, Type=as.factor("Single")),
            aes(xmin=0.8, xmax=1.2, ymin=0.27, ymax=0.65), color="#8BC34A", alpha=0, linewidth = 1.2) +
  geom_rect(data=data.frame(Outcome = "Enticement", Probability=0.1, Type=as.factor("Single")),
            aes(xmin=1.8, xmax=2.2, ymin=0.3, ymax=0.68), color="#8BC34A", alpha=0, linewidth = 1.2) +
  geom_rect(data=data.frame(Outcome = "Enticement", Probability=0.1, Type=as.factor("Single")),
            aes(xmin=2.8, xmax=3.2, ymin=0.33, ymax=0.71), color="#8BC34A", alpha=0, linewidth = 1.2) +
  facet_grid(~Type, switch="y", scales ="free") +
  scale_y_continuous(limits=c(0, 1)) +
  scale_color_manual(values=c("Male"="#147AFF", "Female"="#FF9914")) +
  theme_minimal() +
    theme(axis.title.y = element_blank(),
        axis.title.x = element_blank(),
        axis.text.y = element_text(size = 10, face = "bold"),
...
ggsave("../../paper/images/Figure5.png", type_fig, width = 10, height = 12, dpi = 300)
...
ggsave("../../paper/images/Figure7.png", p2, width = 10, height = 12, dpi = 300)
